...
class SyncManager:
    ...
    def full_rescan(self):
        ...
        with self._lock:
            ...
            try:
                ...
                # Attach similarity, summary, and top file to folder data
                cluster_sims = self.engine.get_cluster_similarities()
                cluster_summaries = self.engine.get_cluster_summaries()
                cluster_top_files = self.engine.get_cluster_top_files()
                folder_to_cluster = {}
                for cid, cdata in clusters.items():
                    folder_to_cluster[cdata["label"]] = cid
                for folder in folder_data:
                    cid = folder_to_cluster.get(folder["name"])
                    if cid is not None:
                        folder["similarity"] = cluster_sims.get(cid)
                        summary_lines = cluster_summaries.get(cid, [])
                        folder["summary"] = summary_lines[:2] if summary_lines else []
                        folder["top_file"] = cluster_top_files.get(cid)
                    else:
                        folder["similarity"] = None
                        folder["summary"] = []
                        folder["top_file"] = None
    ...
    def get_folders(self) -> list:
        """Return managed folder structure with similarity, summary, and top file."""
        folders = self.folder_manager.get_managed_folders()
        cluster_sims = self.engine.get_cluster_similarities()
        cluster_summaries = self.engine.get_cluster_summaries()
        cluster_top_files = self.engine.get_cluster_top_files()
        clusters = self.engine.get_clusters()
        folder_to_cluster = {}
        for cid, cdata in clusters.items():
            folder_to_cluster[cdata["label"]] = cid
        for folder in folders:
            ...
            if cid is not None:
                ...
                summary_lines = cluster_summaries.get(cid, [])
                folder["summary"] = summary_lines[:2] if summary_lines else []
                top_file = cluster_top_files.get(cid)
                folder["top_file"] = top_file
            else:
                folder["similarity"] = None
                folder["summary"] = []
                folder["top_file"] = None
