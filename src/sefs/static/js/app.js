...
(function () {
    ...
    let showEdges = true;
    let width, height;
    let clusterSummaryMap = {};  // cluster_id -> summary lines
    ...
    function renderFolderTree(folders) {
        ...
        folders.forEach((folder, idx) => {
            ...
            const simPct = folder.similarity != null ? Math.round(folder.similarity) : 0;
            const barWidth = Math.max(0, Math.min(100, simPct));

            // Top similar file
            const topFile = folder.top_file;
            const topFileName = topFile ? topFile.name : '';
            const topFileSim = topFile ? `${topFile.similarity}%` : '';
            ...
            html += `
                <div class="folder-card" style="--card-color: ${color}">
                    <div class="folder-card-header" data-folder="${escapedName}">
                        <div class="folder-card-title-row">
                            <span class="folder-card-dot" style="background: ${color}; color: ${color}"></span>
                            <span class="folder-card-name" title="${folder.name}">${folder.name}</span>
                            <span class="folder-card-count">${folder.file_count} file${folder.file_count !== 1 ? 's' : ''}</span>
                            <span class="folder-card-chevron">&#9654;</span>
                        </div>
                        <div class="folder-card-bar-wrap">
                            <div class="folder-card-bar">
                                <div class="folder-card-bar-fill" style="width: ${barWidth}%; background: ${color}"></div>
                            </div>
                            <span class="folder-card-bar-pct" style="color: ${color}">${simPct}%</span>
                        </div>
                        <div class="folder-card-stats">
                        ${topFileName ? `
                        <div class="folder-card-topfile">
                            <span class="topfile-icon">&#11088;</span>
                            <span class="topfile-label">Top:</span>
                            <span class="topfile-name" title="${topFileName}">${topFileName}</span>
                            <span class="topfile-sim">${topFileSim}</span>
                        </div>` : ''}
                        </div>
                        ${(line1 || line2) ? `
                        <div class="folder-card-summary">
                            ${line1 ? `<div class="summary-line">${escapeHtml(line1)}</div>` : ''}
                            ${line2 ? `<div class="summary-line">${escapeHtml(line2)}</div>` : ''}
                        </div>` : ''}
                    </div>
                    <div class="folder-card-files">`;
        ...
        tree.querySelectorAll('.folder-card-header').forEach(header => {
            header.addEventListener('click', () => {
                const card = header.closest('.folder-card');
                const files = card.querySelector('.folder-card-files');
                const chevron = header.querySelector('.folder-card-chevron');
    ...
    function renderGraph() {
        ...
        // Build cluster summary map from node data
        clusterSummaryMap = {};
        graphData.nodes.forEach(n => {
            if (n.cluster_summary && n.cluster_summary.length > 0 && !clusterSummaryMap[n.cluster]) {
                clusterSummaryMap[n.cluster] = n.cluster_summary;
            }
        });
        ...
        // --- Compute cluster centroids for initial positioning ---
        ...
        clusterIds.forEach((cid, i) => {
            clusterAngle[cid] = i * angleStep;
        });

        // Position nodes toward their cluster's region
        nodes.forEach(n => {
            const angle = clusterAngle[n.cluster] || 0;
            const jitter = (Math.random() - 0.5) * 80;
            n.x = width / 2 + Math.cos(angle) * spreadRadius + jitter;
            n.y = height / 2 + Math.sin(angle) * spreadRadius + jitter;
        });

        // Custom cluster force: pull same-cluster nodes together, push different clusters apart
        function forceCluster(alpha) {
            ...
            const cids = Object.keys(centroids);
            const minDist = spreadRadius * 0.8;
            for (let i = 0; i < cids.length; i++) {
                for (let j = i + 1; j < cids.length; j++) {
                    const ci = centroids[cids[i]], cj = centroids[cids[j]];
                    let dx = ci.x - cj.x;
                    let dy = ci.y - cj.y;
                    ...
                    if (dist < minDist) {
                        ...
                        const fx = dx * force;
                        const fy = dy * force;
                        // Push all nodes in each cluster apart
                        nodes.forEach(n => {
                            if (String(n.cluster) === cids[i]) { n.vx += fx; n.vy += fy; }
                            if (String(n.cluster) === cids[j]) { n.vx -= fx; n.vy -= fy; }
                        });
                    }
                }
            }

            // Pull nodes gently toward their cluster centroid
            const pullStrength = 0.15;
            nodes.forEach(n => {
                const c = centroids[n.cluster];
                n.vx += (c.x - n.x) * alpha * pullStrength;
                n.vy += (c.y - n.y) * alpha * pullStrength;
            });
        }
        ...
        simulation = d3.forceSimulation(nodes)
            .force('link', d3.forceLink(edges)
                .id(d => d.id)
                .distance(d => 100 * (1 - (d.weight || 0)) + 40)
                .strength(d => (d.weight || 0.1) * 0.3))
            ...
            .force('x', d3.forceX(d => {
                ...
                return width / 2 + Math.cos(angle) * spreadRadius;
            }).strength(0.06))
            .force('y', d3.forceY(d => {
                const angle = clusterAngle[d.cluster] || 0;
                return height / 2 + Math.sin(angle) * spreadRadius;
            }).strength(0.06))
    ...
    function updateHulls(nodes) {
        ...
        Object.entries(clusterGroups).forEach(([clusterId, clusterNodes]) => {
            // Compute bounding-box centroid of cluster
            ...
            if (clusterNodes.length >= 3) {
                // Add padding points around each node to create a rounder hull
                const paddedPoints = [];
                const pad = 45;
                clusterNodes.forEach(n => {
                    paddedPoints.push([n.x - pad, n.y - pad]);
                    paddedPoints.push([n.x + pad, n.y - pad]);
                    paddedPoints.push([n.x - pad, n.y + pad]);
                    paddedPoints.push([n.x + pad, n.y + pad]);
                    paddedPoints.push([n.x, n.y - pad]);
                    paddedPoints.push([n.x, n.y + pad]);
                    paddedPoints.push([n.x - pad, n.y]);
                    paddedPoints.push([n.x + pad, n.y]);
                });
                try {
        ...
        hullData.forEach(d => {
            let hullEl;
            if (d.path) {
                // Use a smooth curve path via cardinal spline for rounded hulls
                const line = d3.line().curve(d3.curveCatmullRomClosed.alpha(0.8));
                hullEl = hullGroup.append('path')
                ...
                const rx = Math.abs(n1.x - n2.x) / 2 + 55;
                const ry = Math.max(Math.abs(n1.y - n2.y) / 2 + 45, 50);

                hullEl = hullGroup.append('ellipse')
                    .attr('class', 'cluster-hull')
                    ...
                    .attr('stroke-opacity', 0.2)
                    .attr('stroke-width', 1.5);
            } else if (d.singleNode) {
                hullEl = hullGroup.append('circle')
                    .attr('class', 'cluster-hull')
                    .attr('cx', d.singleNode.x)
                    .attr('cy', d.singleNode.y)
                    .attr('r', 50)
                    .attr('fill', d.color)
                    .attr('fill-opacity', 0.06)
                    .attr('stroke', d.color)
            ...
            // Attach cluster hover tooltip to hull shape
            if (hullEl) {
                hullEl
                    .style('pointer-events', 'all')
                    .style('cursor', 'default')
                    .on('mouseover', (event) => {
                        const ct = document.getElementById('cluster-tooltip');
                        const summary = clusterSummaryMap[parseInt(d.id)] || [];
                        const nameEl = document.getElementById('ct-name');
                        const linesEl = document.getElementById('ct-lines');
                        ...
                        if (summary.length === 0) {
                            const li = document.createElement('div');
                            li.className = 'ct-line';
                            li.textContent = 'No summary available';
                            linesEl.appendChild(li);
                        }
                    ...
                    .on('mousemove', (event) => {
                        ...
                        const maxX = window.innerWidth - ct.offsetWidth - 20;
                        const maxY = window.innerHeight - ct.offsetHeight - 20;
                        ct.style.left = Math.min(x, maxX) + 'px';
                        ct.style.top = Math.min(y, maxY) + 'px';
                    })
                    .on('mouseout', () => {
                        document.getElementById('cluster-tooltip').classList.remove('visible');
                    });
            }

            // Cluster label positioned above hull
            const labelY = d.path
                ? Math.min(...d.path.map(p => p[1])) - 14
                : d.centroid[1] - 60;
