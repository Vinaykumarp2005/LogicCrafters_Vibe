...
class SemanticEngine:
    ...
    def get_cluster_summaries(self) -> dict:
        """
        Generate a plain 3-line summary for each cluster from its files' content.
        Returns dict of cluster_id -> list of 3 summary sentences.
        """
        summaries = {}
        for cid, cluster_data in self._clusters.items():
            files = cluster_data.get("files", [])
            label = cluster_data.get("label", f"Cluster {cid}")

            # Gather content and extract meaningful sentences
            all_sentences = []
            for fp in files:
                fd = self._file_data.get(fp, {})
                text = fd.get("content", "").strip()
                if text:
                    # Split into sentences, keep those with real info
                    raw = re.split(r'(?<=[.!?])\s+', text[:2000])
                    for s in raw:
                        s = s.strip()
                        # Keep sentences that are informative (10-200 chars, has letters)
                        if 10 < len(s) < 200 and re.search(r'[a-zA-Z]{3,}', s):
                            all_sentences.append(s)

            lines = []
            if all_sentences:
                # Pick 3 diverse sentences: use keyword scoring to find the most representative
                stop = {
                    "the", "and", "for", "are", "but", "not", "you", "all",
                    "can", "had", "her", "was", "one", "our", "out", "has",
                    "have", "each", "make", "like", "been", "from", "this",
                    "that", "with", "they", "will", "would", "there", "their",
                    "what", "about", "which", "when", "were", "them", "than",
                    "into", "more", "other", "some", "such", "also", "these",
                    "just", "over", "very", "then", "only", "come", "could",
                    "being", "after", "most", "should", "does", "much",
                }
                # Build keyword frequency from all content
                all_words = []
                for s in all_sentences:
                    all_words.extend(w.lower() for w in re.findall(r'\b[a-zA-Z]{4,}\b', s))
                freq = Counter(w for w in all_words if w not in stop)

                # Score each sentence by sum of its keyword frequencies
                scored = []
                for s in all_sentences:
                    words = [w.lower() for w in re.findall(r'\b[a-zA-Z]{4,}\b', s)]
                    score = sum(freq.get(w, 0) for w in words if w not in stop)
                    scored.append((score, s))

                # Sort by score descending, pick top 3 non-overlapping
                scored.sort(key=lambda x: -x[0])
                selected = []
                for _, s in scored:
                    # Skip if too similar to already selected
                    s_lower = s.lower()
                    if any(s_lower[:30] == prev.lower()[:30] for prev in selected):
                        continue
            ...
            if len(lines) == 0:
                lines = [
                    f"A collection of {len(files)} related file{'s' if len(files) != 1 else ''}.",
                    f"Grouped under the {label} topic.",
                    "Content details are not available."
                ]
            elif len(lines) == 1:
                lines.append(f"Contains {len(files)} semantically related file{'s' if len(files) != 1 else ''}.")
                lines.append(f"Cluster topic: {label}.")
            elif len(lines) == 2:
                lines.append(f"Contains {len(files)} file{'s' if len(files) != 1 else ''} on this topic.")

            summaries[cid] = lines[:3]
    ...
    def get_cluster_similarities(self) -> dict:
        """
        Compute average intra-cluster similarity percentage for each cluster.
        Returns dict of cluster_id -> float (0-100).
        """
        from sklearn.metrics.pairwise import cosine_similarity
        ...
        for cid, cluster_data in self._clusters.items():
            ...
            vectors = []
            for fp in files:
                if fp in self._embeddings:
                    vectors.append(self._embeddings[fp])
            ...
            for i in range(n):
                for j in range(i + 1, n):
                    ...
                    count += 1
            avg_sim = (total / count) * 100 if count > 0 else 100.0
            result[cid] = round(avg_sim, 1)

        return result

    def get_cluster_top_files(self) -> dict:
        """
        For each cluster, find the file with the highest average similarity
        to all other files in the cluster (the most representative file).
        Returns dict of cluster_id -> {"name": str, "path": str, "similarity": float}.
        """
        from sklearn.metrics.pairwise import cosine_similarity
        ...
        for cid, cluster_data in self._clusters.items():
            files = cluster_data.get("files", [])
            file_paths = [fp for fp in files if fp in self._embeddings]
            ...
            vecs = np.array([self._embeddings[fp] for fp in file_paths])
            sim_matrix = cosine_similarity(vecs)

            # Average similarity of each file to all others
            best_idx = 0
            best_avg = -1
            for i in range(len(file_paths)):
                avg = sum(sim_matrix[i][j] for j in range(len(file_paths)) if j != i) / (len(file_paths) - 1)
                if avg > best_avg:
                    best_avg = avg
                    best_idx = i

            fp = file_paths[best_idx]
            result[cid] = {
                "name": Path(fp).name,
                "path": fp,
                "similarity": round(best_avg * 100, 1),
            }
    ...
    def get_graph_data(self) -> dict:
        ...
        # Attach cluster summaries
        cluster_summaries = self.get_cluster_summaries()
        for node in nodes:
            cid = node["cluster"]
            node["cluster_summary"] = cluster_summaries.get(cid, [])
